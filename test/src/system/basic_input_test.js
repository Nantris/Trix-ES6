 ...
 */
const {assert, defer, dragToCoordinates, expandSelection, insertNode, moveCursor, pressKey, selectAll, test, testGroup, triggerEvent, typeCharacters} = Trix.TestHelpers;

testGroup("Basic input", {template: "editor_empty"}, function() {
  test("typing", expectDocument =>
    typeCharacters("abc", () => expectDocument("abc\n"))
  );

  test("backspacing", expectDocument =>
    typeCharacters("abc\b", function() {
  ...
  test("pressing delete", expectDocument =>
    typeCharacters("ab", () =>
      moveCursor("left", () =>
        pressKey("delete", () => expectDocument("a\n"))
      )
    )
  );
  ...
  test("pressing return", expectDocument =>
    typeCharacters("ab", () =>
      pressKey("return", () =>
        typeCharacters("c", () => expectDocument("ab\nc\n"))
      )
    )
  );
  ...
  test("cursor left", expectDocument =>
    typeCharacters("ac", () =>
      moveCursor("left", () =>
        typeCharacters("b", () => expectDocument("abc\n"))
      )
    )
  );
  ...
  test("replace entire document", expectDocument =>
    typeCharacters("abc", () =>
      selectAll(() =>
        typeCharacters("d", () => expectDocument("d\n"))
      )
    )
  );
  ...
  test("remove entire document", expectDocument =>
    typeCharacters("abc", () =>
      selectAll(() =>
        typeCharacters("\b", () => expectDocument("\n"))
      )
    )
  );
      ...
      moveCursor({direction: "left", times: 2}, coordinates =>
        moveCursor("right", () =>
          expandSelection("right", () =>
            dragToCoordinates(coordinates, () => expectDocument("acb\n"))
          )
        )
      )
    )
  ...
  return test("inserting ó with control + alt + o (AltGr)", expectDocument =>
    typeCharacters("ab", () =>
      moveCursor("left", function() {
