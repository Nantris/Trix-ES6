 ...
 */
const {assert, clickToolbarButton, expandSelection, moveCursor, test, testGroup, typeCharacters} = Trix.TestHelpers;
...
testGroup("Undo/Redo", {template: "editor_empty"}, function() {
  ...
  return test("formatting changes are batched by location range", done =>
    typeCharacters("abc", function() {
      const first = getDocument().copy();
      return expandSelection("left", () =>
