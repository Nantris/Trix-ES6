 ...
 */
const {assert, clickToolbarButton, clickToolbarDialogButton, collapseSelection, expandSelection, insertString, insertText, isToolbarButtonActive, isToolbarButtonDisabled, isToolbarDialogActive, moveCursor, pressKey, test, testGroup, typeCharacters, typeInToolbarDialog, typeToolbarKeyCommand} = Trix.TestHelpers;
...
testGroup("Text formatting", {template: "editor_empty"}, function() {
  ...
  test("applying a link to text", done =>
    typeCharacters("abc", () =>
      moveCursor("left", () =>
        expandSelection("left", () =>
          clickToolbarButton({attribute: "href"}, function() {
  ...
  test("editing a link", function(done) {
    insertString("a");
    const text = Trix.Text.textForStringWithAttributes("bc", {href: "http://example.com"});
    insertText(text);
    insertString("d");
    return moveCursor({direction: "left", times: 2}, () =>
      clickToolbarButton({attribute: "href"}, function() {
        assert.ok(isToolbarDialogActive({attribute: "href"}));
  ...
  test("removing a link", function(done) {
    const text = Trix.Text.textForStringWithAttributes("ab", {href: "http://example.com"});
    insertText(text);
    assert.textAttributes([0, 2], {href: "http://example.com"});
    return expandSelection({direction: "left", times: 2}, () =>
      clickToolbarButton({attribute: "href"}, () =>
        clickToolbarDialogButton({method: "removeAttribute"}, function() {
          assert.textAttributes([0, 2], {});
          return done();
        })
      )
  ...
  test("applying formatting to an unfocused editor", function(done) {
    const input = Trix.makeElement("input", {type: "text"});
    document.body.appendChild(input);
    input.focus();

    return clickToolbarButton({attribute: "bold"}, () =>
  ...
  test("key command activates toolbar button", done =>
    typeToolbarKeyCommand({attribute: "bold"}, function() {
      assert.ok(isToolbarButtonActive({attribute: "bold"}));
      return done();
    })
  );

  return test("backspacing newline after text", expectDocument =>
    typeCharacters("a\n", () =>
