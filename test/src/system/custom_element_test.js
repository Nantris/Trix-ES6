 ...
 */
const {after, assert, clickElement, clickToolbarButton, createFile, defer, insertImageAttachment, moveCursor, pasteContent, skip, test, testIf, testGroup, triggerEvent, typeCharacters, typeInToolbarDialog} = Trix.TestHelpers;
...
testGroup("Custom element API", {template: "editor_empty"}, function() {
  ...
  test("element triggers toolbar dialog events", function(done) {
    ...
    return clickToolbarButton({action: "link"}, () =>
  ...
  test("element triggers before-paste event with paste data", function(expectDocument) {
    ...
    return typeCharacters("", () =>
      pasteContent("text/html", "<strong>hello</strong>", function() {
        assert.equal(eventCount, 1);
