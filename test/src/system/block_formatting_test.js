 ...
 */
const {assert, clickToolbarButton, defer, expandSelection, isToolbarButtonActive, isToolbarButtonDisabled, moveCursor, pressKey, replaceDocument, selectAll, test, testGroup, typeCharacters} = Trix.TestHelpers;
...
`, () =>
      moveCursor({direction: "left", times: 2}, () =>
        expandSelection({direction: "left", times: 15}, () =>
          clickToolbarButton({attribute: "bullet"}, function() {
  ...
  test("applying block attributes to adjacent unformatted blocks consolidates them", function(done) {
    const document = new Trix.Document([
  ...
  test("breaking out of a formatted block with adjacent non-formatted blocks", function(expectDocument) {
    ...
    // b*
    // c
    let document = new Trix.Document([
        new Trix.Block(Trix.Text.textForStringWithAttributes("a"), []),
  ...
  test("increasing list level", function(done) {
    ...
    return clickToolbarButton({attribute: "bullet"}, function() {
      ...
      return typeCharacters("a\n", function() {
        ...
        assert.notOk(isToolbarButtonDisabled({action: "decreaseNestingLevel"}));
        return clickToolbarButton({action: "increaseNestingLevel"}, () =>
          typeCharacters("b", function() {
            assert.ok(isToolbarButtonDisabled({action: "increaseNestingLevel"}));
            assert.notOk(isToolbarButtonDisabled({action: "decreaseNestingLevel"}));
  ...
  test("removing bullet from heading block", done =>
    clickToolbarButton({attribute: "bullet"}, () =>
      clickToolbarButton({attribute: "heading1"}, function() {
  ...
  test("breaking out of middle of heading block", expectDocument =>
    clickToolbarButton({attribute: "heading1"}, () =>
      typeCharacters("abc", function() {
        assert.ok(isToolbarButtonActive({attribute: "heading1"}));
        return moveCursor({direction: "left", times: 1}, () =>
          typeCharacters("\n", function() {
            const document = getDocument();
            assert.equal(document.getBlockCount(), 2);
  ...
  test("terminal attributes replace existing terminal attributes", function(expectDocument) {
    ...
    return selectAll(() =>
      clickToolbarButton({attribute: "code"}, function() {
        assert.equal(getDocument().getBlockCount(), 3);
  ...
  test("unindenting a code block inside a bullet", expectDocument =>
    clickToolbarButton({attribute: "bullet"}, () =>
      clickToolbarButton({attribute: "code"}, () =>
        typeCharacters("a", () =>
          clickToolbarButton({action: "decreaseNestingLevel"}, function() {
            const document = getDocument();
  ...
  return test("list indentation constraints consider the list type", expectDocument =>
    clickToolbarButton({attribute: "bullet"}, () =>
      typeCharacters("a\n\n", () =>
