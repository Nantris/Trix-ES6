 ...
 */
const {assert, clickToolbarButton, defer, endComposition, insertNode, pressKey, selectNode, startComposition, test, testIf, testGroup, triggerEvent, typeCharacters, updateComposition} = Trix.TestHelpers;
const {browser} = Trix;
...
testGroup("Composition input", {template: "editor_empty"}, function() {
  test("composing", expectDocument =>
    startComposition("a", () =>
      updateComposition("ab", () =>
        endComposition("abc", () => expectDocument("abc\n"))
      )
    )
  );
    ...
    typeCharacters("a", () =>
      startComposition("b", () =>
        updateComposition("bc", () =>
          endComposition("bcd", () =>
            typeCharacters("e", () => expectDocument("abcde\n"))
          )
        )
      )
    )
  ...
  test("composing another language using a QWERTY keyboard", function(expectDocument) {
    ...
    return startComposition("x", function() {
      ...
      return updateComposition("xi", () =>
        endComposition("喜", () => expectDocument("喜\n"))
      );
    });
  });

  // Simulates the sequence of events when pressing backspace through a word on Android
  test("backspacing through a composition", function(expectDocument) {
    const element = getEditorElement();
  ...
  test("updating a composition", function(expectDocument) {
    const element = getEditorElement();
  ...
  test("leaving a composition", function(expectDocument) {
    const element = getEditorElement();
  ...
  testIf(browser.composesExistingText, "composition events from cursor movement are ignored", function(expectDocument) {
    const element = getEditorElement();
  ...
  return test("composition ending with same data as last update", function(expectDocument) {
    const element = getEditorElement();
