...
testGroup("Trix.LocationMapper", function() {
  test("findLocationFromContainerAndOffset", function() {
    ...
    return (() => {
      const result = [];
      for (let assertion of Array.from(assertions)) {
...
var setDocument = function(json) {
  ...
  return mapper = new Trix.LocationMapper(element);
};

var findContainer = function(path) {
  let el = element;
  for (let index of Array.from(path)) { el = el.childNodes[index]; }
  return el;
};
