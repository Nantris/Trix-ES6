...
testGroup("Trix.summarizeStringChange", function() {
  const assertions = {
    "no change": {
      oldString: "abc",
      newString: "abc",
  ...
  return (() => {
    ...
    for (var name in assertions) {
      ...
      result.push((({oldString, newString, change}) =>
        test(name, () => assert.deepEqual(Trix.summarizeStringChange(oldString, newString), change))
      )(details));
    }
    return result;
  })();
