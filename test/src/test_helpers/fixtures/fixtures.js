 ...
 */
let createCursorTarget;
const {css} = Trix.config;

this.TEST_IMAGE_URL = "data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACwAAAAAAQABAAACAkQBADs=";
...
const createDocument = function(...parts) {
  const blocks = (() => {
    ...
    for (let part of Array.from(parts)) {
      ...
      result.push(new Trix.Block(text, blockAttributes));
    }
    return result;
  })();
...
this.fixtures = {
  ...
  "image attachment": (function() {
    ...
    const image = Trix.makeElement("img", {src: attrs.url, "data-trix-mutable": true, width: 1, height: 1});
    image.dataset.trixStoreKey = ["imageElement", attachment.id, image.src, image.width, image.height].join("/");

    const caption = Trix.makeElement({tagName: "figcaption", className: css.attachmentCaption});
    ...
    for (let key in data) { const value = data[key]; figure.dataset[key] = value; }
  ...
  "text with newlines and image attachment": (function() {
    const stringText = Trix.Text.textForStringWithAttributes("a\nb");

    const attrs = {url: TEST_IMAGE_URL, filename: "example.png", filesize: 98203, contentType: "image/png", width: 1, height: 1};
    const attachment = new Trix.Attachment(attrs);
    const attachmentText = Trix.Text.textForAttachmentWithAttributes(attachment);
  ...
  "image attachment with edited caption": (function() {
    ...
    const text = Trix.Text.textForAttachmentWithAttributes(attachment, textAttrs);

    const image = Trix.makeElement("img", {src: attrs.url, "data-trix-mutable": true, width: 1, height: 1});
    image.dataset.trixStoreKey = ["imageElement", attachment.id, image.src, image.width, image.height].join("/");

    const caption = Trix.makeElement({tagName: "figcaption", className: `${css.attachmentCaption} ${css.attachmentCaption}--edited`, textContent: "Example"});
  ...
  "file attachment": (function() {
    ...
    const link = Trix.makeElement("a", {href: attrs.href, tabindex: -1});
    for (let key in data) { const value = data[key]; link.dataset[key] = value; }
  ...
  "pending file attachment": (function() {
    ...
    for (let key in data) { value = data[key]; figure.dataset[key] = value; }
    figure.setAttribute("contenteditable", false);

    const progress = Trix.makeElement({
      tagName: "progress",
      attributes: {
  ...
  "content attachment": (function() {
    ...
    const attachment = new Trix.Attachment({content, contentType, href});
    const text = Trix.Text.textForAttachmentWithAttributes(attachment);
...
this.eachFixture = function(callback) {
  return (() => {
    const result = [];
    for (let name in this.fixtures) {
      const details = this.fixtures[name];
