 ...
 */
const {spliceArray} = Trix;

(function() {
  let objectArraysAreEqual = undefined;
  let startOfRange = undefined;
  let endOfRange = undefined;
  const Cls = (Trix.SplittableList = class SplittableList extends Trix.Object {
    static initClass() {
      ...
      startOfRange = range => range[0];
  
      endOfRange = range => range[1];
    }
    static box(objects) {
    ...
    consolidate() {
      ...
      for (let object of Array.from(this.objects.slice(1))) {
        if (typeof pendingObject.canBeConsolidatedWith === 'function' ? pendingObject.canBeConsolidatedWith(object) : undefined) {
    ...
    isEqualTo(splittableList) {
      return super.isEqualTo(...arguments) || objectArraysAreEqual(this.objects, splittableList != null ? splittableList.objects : undefined);
    }
